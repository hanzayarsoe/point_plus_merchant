...
import 'package:merchant/core/services/remote_message_model.dart';
...
class FirebaseMessagingService {
  ...
  /// Sets up interactions for messages when the app is in the background or terminated
  ...
  void _onMessageOpenedApp(RemoteMessage message) {
    ...
    final data = RemoteMessageModel.fromJson(message.data);
    final appRouter = sl<AppRouter>();
    if (data.requestId != null) {
      appRouter.router.goNamed(
        AppRoutes.requestTransactionDetail,
        pathParameters: {"id": data.requestId!},
      );
    } else if (data.transactionId != null) {
      appRouter.router.goNamed(
        AppRoutes.transactionDetail,
        pathParameters: {"id": data.transactionId!},
      );
    }
  }
}
